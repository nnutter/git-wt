...
func TestCreateListAndRemoveLifecycle(t *testing.T) {
	...
	createResult := testRepository.runGitWT(t, "create", at(testRepoName, branchName))
	...
	listResult := testRepository.runGitWT(t, "list", at(testRepoName, ""))
	...
	removeResult := testRepository.runGitWT(t, "remove", at(testRepoName, branchName))
...
func TestCreateUsesOriginHeadAsDefaultUpstream(t *testing.T) {
	...
	result := testRepository.runGitWT(t, "create", at(testRepoName, "feature/from-develop"))
...
func TestCreateFallsBackToOriginMasterWhenOriginHeadIsMissing(t *testing.T) {
	...
	result := testRepository.runGitWT(t, "create", at(testRepoName, "feature/from-master"))
	require.NoError(t, result.err, result.stderr)
...
func TestCreateFallsBackToOriginMainWhenOriginHeadAndMasterAreMissing(t *testing.T) {
	...
	result := testRepository.runGitWT(t, "create", at(testRepoName, "feature/from-main"))
	require.NoError(t, result.err, result.stderr)
}
...
func TestCreateFailsWhenOriginHeadAndCommonDefaultsAreMissing(t *testing.T) {
	...
	result := testRepository.runGitWT(t, "create", at(testRepoName, "feature/missing-default-upstream"))
...
func TestCreateWithHerdrOpensStandardHerdrSpace(t *testing.T) {
	...
	result := testRepository.runGitWT(t, "create", "--herdr", at(testRepoName, branchName))
	require.NoError(t, result.err, result.stderr)
	testRepository.assertPathPresent(t, testRepository.worktreePath(branchName))
...
func TestCreateWithNoHerdrDoesNotInvokeHerdr(t *testing.T) {
	...
	result := testRepository.runGitWT(t, "create", "--no-herdr", at(testRepoName, branchName))
	require.NoError(t, result.err, result.stderr)
	_, err := os.Stat(logPath)
	assert.True(t, os.IsNotExist(err))
}
...
func TestTUICreateListsEveryRepositoryFromAManagedWorktree(t *testing.T) {
	...
	require.NoError(t, testRepository.runGitWT(t, "create", at(testRepoName, currentBranch)).err)
...
func TestSetupSpaceOpensNamedWorktreeInNewHerdrWorkspace(t *testing.T) {
	...
	testRepository := newTestRepository(t)
	require.NoError(t, testRepository.runGitWT(t, "create", at(testRepoName, branchName)).err)

	logPath := filepath.Join(t.TempDir(), "herdr.log")
	installFakeHerdrSpace(t, logPath)

	result := testRepository.runGitWT(t, "setup-space", "--new", at(testRepoName, branchName))
	require.NoError(t, result.err, result.stderr)
	assert.Contains(t, result.stderr, "opened herdr space for "+branchName)
...
func TestSetupSpaceDefinesNamedWorktreeTabsInCurrentHerdrSpace(t *testing.T) {
	...
	result := testRepository.runGitWT(t, "setup-space", at(testRepoName, branchName))
	require.NoError(t, result.err, result.stderr)
...
func TestSetupSpaceFailsForUnknownWorktree(t *testing.T) {
	...
	result := testRepository.runGitWT(t, "setup-space", at(testRepoName, "feature/missing"))
	require.Error(t, result.err)
...
func TestSetupSpaceRequiresNameOutsideManagedWorktree(t *testing.T) {
	...
	result := testRepository.runGitWT(t, "setup-space", at(testRepoName, ""))
	require.Error(t, result.err)
...
func TestRemoveRemovesEmptyParentDirectories(t *testing.T) {
	...
	result := testRepository.runGitWT(t, "remove", at(testRepoName, branchName))
	require.NoError(t, result.err, result.stderr)
...
func TestRemoveForceRemovesDirtyUnmergedWorktree(t *testing.T) {
	...
	result := testRepository.runGitWT(t, "remove", "--force", at(testRepoName, branchName))
	require.NoError(t, result.err, result.stderr)
	testRepository.assertPathMissing(t, testRepository.worktreePath(branchName))
	testRepository.assertBranchMissing(t, branchName)
}
...
func TestRemoveCompletionOffersManagedWorktreeNames(t *testing.T) {
	testRepository := newTestRepository(t)
	require.NoError(t, testRepository.runGitWT(t, "create", at(testRepoName, "feature/a")).err)
	require.NoError(t, testRepository.runGitWT(t, "create", at(testRepoName, "feature/b")).err)
...
func TestSwitchCompletionOffersWorktreeNamesAcrossRepos(t *testing.T) {
	...
	require.NoError(t, primary.runGitWT(t, "create", at(testRepoName, "feature/current")).err)
	require.NoError(t, primary.runGitWT(t, "create", at(secondaryName, "feature/other")).err)
...
func TestSwitchResolvesPathWithRepoFlag(t *testing.T) {
	...
	result := testRepository.runGitWT(t, "switch", at(testRepoName, branchName))
	require.NoError(t, result.err, result.stderr)
...
func TestSwitchResolvesRepoFromCurrentWorktree(t *testing.T) {
	testRepository := newTestRepository(t)
	require.NoError(t, testRepository.runGitWT(t, "create", at(testRepoName, "feature/from")).err)
	require.NoError(t, testRepository.runGitWT(t, "create", at(testRepoName, "feature/to")).err)
...
func TestSwitchInfersUniqueRepoOutsideWorktree(t *testing.T) {
	...
	require.NoError(t, primary.runGitWT(t, "create", at(testRepoName, "feature/login")).err)
...
func TestSwitchRequiresRepoWhenWorktreeNameIsAmbiguous(t *testing.T) {
	...
	registerAdditionalRepo(t, primary, secondaryName)
	require.NoError(t, primary.runGitWT(t, "create", at(testRepoName, "feature/login")).err)
	require.NoError(t, primary.runGitWT(t, "create", at(secondaryName, "feature/login")).err)
...
func TestSwitchInfersUniqueRepoInsideWorktree(t *testing.T) {
	...
	result := primary.runGitWTFrom(t, primary.worktreePath("feature/current"), "switch", "feature/other")
	require.NoError(t, result.err, result.stderr)
...
func TestSwitchFailsWhenWorktreeMissing(t *testing.T) {
	...
	result := testRepository.runGitWT(t, "switch", at(testRepoName, "missing"))
	require.Error(t, result.err)
	assert.Contains(t, result.err.Error(), "not found")
}
...
func TestSwitchCreateCreatesWorktreeAndReportsPath(t *testing.T) {
	...
	testRepository := newTestRepository(t)
	result := testRepository.runGitWT(t, "switch", "-c", at(testRepoName, branchName))
	require.NoError(t, result.err, result.stderr)
	testRepository.assertPathPresent(t, testRepository.worktreePath(branchName))
	assert.Equal(t, testRepository.worktreePath(branchName), strings.TrimSpace(result.stdout))
...
func TestSwitchCreateAcceptsFlagAfterName(t *testing.T) {
	...
	testRepository := newTestRepository(t)
	result := testRepository.runGitWT(t, "switch", at(testRepoName, branchName), "--create")
	require.NoError(t, result.err, result.stderr)
	testRepository.assertPathPresent(t, testRepository.worktreePath(branchName))
	assert.Equal(t, testRepository.worktreePath(branchName), strings.TrimSpace(result.stdout))
}
...
func TestSwitchCreateFailsWhenWorktreeExists(t *testing.T) {
	...
	pathFile := filepath.Join(t.TempDir(), "switch-path")
	t.Setenv(switchPathFileEnvVarName, pathFile)
	result := testRepository.runGitWT(t, "switch", at(testRepoName, branchName), "-c")
	require.Error(t, result.err)
	assert.Contains(t, result.err.Error(), "already exists")
...
func TestSwitchCreateNoCdDoesNotReportPath(t *testing.T) {
	...
	result := testRepository.runGitWT(t, "switch", "-c", "--no-cd", at(testRepoName, branchName))
	require.NoError(t, result.err, result.stderr)
	testRepository.assertPathPresent(t, testRepository.worktreePath(branchName))
	assert.Empty(t, result.stdout)
	_, err := os.Stat(pathFile)
...
func TestSwitchCreateWithHerdrDoesNotReportPath(t *testing.T) {
	...
	result := testRepository.runGitWT(t, "switch", "-c", "--herdr", at(testRepoName, branchName))
	require.NoError(t, result.err, result.stderr)
	testRepository.assertPathPresent(t, testRepository.worktreePath(branchName))
...
func TestPruneRemovesOnlyMergedCleanWorktrees(t *testing.T) {
	...
	require.NoError(t, testRepository.runGitWT(t, "create", at(testRepoName, "feature/merged")).err)
	testRepository.mergeWorktreeBranch(t, "feature/merged")

	require.NoError(t, testRepository.runGitWT(t, "create", at(testRepoName, "feature/unmerged")).err)
	testRepository.commitFileInWorktree(t, "feature/unmerged", "extra.txt", "extra\n")

	require.NoError(t, testRepository.runGitWT(t, "create", at(testRepoName, "feature/dirty")).err)
	testRepository.mergeWorktreeBranch(t, "feature/dirty")
	testRepository.writeFileInWorktree(t, "feature/dirty", "dirty.txt", "dirty\n")

	result := testRepository.runGitWT(t, "prune", at(testRepoName, ""))
	require.NoError(t, result.err, result.stderr)
...
func TestPruneDryRunListsAndKeepsWorktrees(t *testing.T) {
	...
	for _, flag := range []string{"-n", "--dry-run"} {
		result := testRepository.runGitWT(t, "prune", flag, at(testRepoName, ""))
		require.NoError(t, result.err, result.stderr)
...
func TestPruneDryRunSucceedsWhenNothingToPrune(t *testing.T) {
	...
	result := testRepository.runGitWT(t, "prune", "--dry-run", at(testRepoName, ""))
	require.NoError(t, result.err, result.stderr)
...
func TestPruneWithoutRepoFromOutsidePrunesAllRepos(t *testing.T) {
	...
	require.NoError(t, primary.runGitWT(t, "create", at(testRepoName, "feature/merged")).err)
	primary.mergeWorktreeBranch(t, "feature/merged")
	require.NoError(t, primary.runGitWT(t, "create", at(secondaryName, "feature/other-merged")).err)
...
func TestPrunePromptDistinguishesSameNameInTwoRepos(t *testing.T) {
	...
	require.NoError(t, primary.runGitWT(t, "create", at(testRepoName, branchName)).err)
	require.NoError(t, primary.runGitWT(t, "create", at(secondaryName, branchName)).err)
...
func TestListSucceedsWhenUpstreamRefIsMissing(t *testing.T) {
	...
	result := testRepository.runGitWT(t, "list", at(testRepoName, ""))
	require.NoError(t, result.err, result.stderr)
...
func TestCreateRepairsBareRepoMissingOriginFetch(t *testing.T) {
	...
	result := testRepository.runGitWT(t, "create", at(testRepoName, "feature/repaired-upstream"))
	require.NoError(t, result.err, result.stderr)
...
func TestCreateWritesPathFileWhenRequested(t *testing.T) {
	...
	result := testRepository.runGitWT(t, "create", at(testRepoName, "feature/path-file"))
	require.NoError(t, result.err, result.stderr)
...
func TestRepoRenameMovesManagedWorktreesAndPreservesUnmanagedWorktrees(t *testing.T) {
	...
	listResult := runGitWTCommand(t, "list", at(newRepoName, ""))
	require.NoError(t, listResult.err, listResult.stderr)
	assert.Contains(t, listResult.stdout, branchName)
}
...
func TestRepoRenameWithoutWorktrees(t *testing.T) {
	...
	oldResult := testRepository.runGitWT(t, "list", at(testRepoName, ""))
...
func TestRepoRemoveRefusesWhenWorktreesExist(t *testing.T) {
	testRepository := newTestRepository(t)
	require.NoError(t, testRepository.runGitWT(t, "create", at(testRepoName, "feature/keep")).err)
...
func TestCreateAutoDetectsRepoFromManagedWorktree(t *testing.T) {
	...
	testRepository := newTestRepository(t)
	require.NoError(t, testRepository.runGitWT(t, "create", at(testRepoName, existing)).err)
...
func TestCreateRejectsAtInWorktreeName(t *testing.T) {
	testRepository := newTestRepository(t)
	result := testRepository.runGitWT(t, "create", "foo@bar@"+testRepoName)
	require.Error(t, result.err)
...
func TestCreateRejectsUnknownRepoQualifier(t *testing.T) {
	testRepository := newTestRepository(t)
	result := testRepository.runGitWT(t, "create", "feature/login@unknown")
	require.Error(t, result.err)
...
func TestListOutsideManagedWorktreeListsAllRepos(t *testing.T) {
	...
	require.NoError(t, primary.runGitWT(t, "create", at(testRepoName, "feature/primary")).err)
	require.NoError(t, primary.runGitWT(t, "create", at(secondaryName, "feature/secondary")).err)
...
func TestListInsideManagedWorktreeListsAllRepos(t *testing.T) {
	...
	result := primary.runGitWTFrom(t, primary.worktreePath("feature/primary"), "list")
	require.NoError(t, result.err, result.stderr)
	assert.Contains(t, result.stdout, "feature/primary")
	assert.Contains(t, result.stdout, "feature/secondary")
	assert.Contains(t, result.stdout, secondaryName)
...
func TestWorktreeCompletionAddsAtWhenNameIsAmbiguous(t *testing.T) {
	...
	require.NoError(t, primary.runGitWT(t, "create", at(testRepoName, "feature/login")).err)
	require.NoError(t, primary.runGitWT(t, "create", at(secondaryName, "feature/login")).err)
	require.NoError(t, primary.runGitWT(t, "create", at(testRepoName, "feature/unique")).err)
...
func TestRemoveInfersUniqueRepoOutsideWorktree(t *testing.T) {
	...
	result := primary.runGitWTFrom(t, primary.home, "remove", "feature/login")
	require.NoError(t, result.err, result.stderr)
...
func TestMigrateRegistersBareAndRehomesWorktrees(t *testing.T) {
	...
	listResult := runGitWTCommand(t, "list", at("project", ""))
	require.NoError(t, listResult.err, listResult.stderr)
	...
	createResult := runGitWTCommand(t, "create", at("project", "feature/after-migrate"))
	require.NoError(t, createResult.err, createResult.stderr)
}
